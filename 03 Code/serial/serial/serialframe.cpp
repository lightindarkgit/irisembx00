...
//{
//    if(NULL == _dataBuf || NULL == pFrameBuf)
//    {
...
{
    if(NULL == pRawDataBuf || NULL == pFrameBuf)
    {
        return E_SERIAL_INVALID_PTR;
    }
...
int SerialFrame::GetFrame(unsigned char* pFrameBuf)
{
    ...
    while(!_isEmpty)
    {
        ...
        //所有数据均遍历完，也没有找到帧头，也即缓冲区此时已出于逻辑空状态
        if(_isEmpty)
        {
            return E_SERIAL_NO_FRAME;
        }
