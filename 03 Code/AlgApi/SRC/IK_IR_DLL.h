...
typedef struct MatchOutputStruct
{
	...
	//NOTE:Use this parameter to judge weather there is a Match
	int matchNum;

	//Index of matched template in the enrolled template list. 
	//NOTE:NEVER Use this parameter (e.g., matchIndex[0]) to judge weather there is a Match
	long matchIndex[IR_MAX_NUM_OF_MATCH];

	//Match score of each match, ranging from 0 to 1, 
	//and the higher the more confidence of the match.
	float matchScore[IR_MAX_NUM_OF_MATCH];

	//Confidence level of each match, ranging from 0 to 1.
	float matchConfidence[IR_MAX_NUM_OF_MATCH];
	...
	float matchAng[IR_MAX_NUM_OF_MATCH];

	// Percent of the area of valid iris image regions possible for iris matching.
	float validROIPercent[IR_MAX_NUM_OF_MATCH];	
	...
	// will be returned.
	int matchLevel[IR_MAX_NUM_OF_MATCH];

	// Number of matched bits.
	int matchedBits[IR_MAX_NUM_OF_MATCH];

	// The total number of valid bits involving iris matching.
	int validMatchBits[IR_MAX_NUM_OF_MATCH];
...
extern "C" IK_IR_API
	//NOTE: the (0,0) point of surImage must be in the up-left corner in a up-right view angle.
	//NOTE: the (0,0) point of pIrisImageL and pIrisImageR is in the down-left corner in a up-right view angle
	int IKIR_IrisDetection(HANDLE hIKIRHandle, unsigned char* surImage, int surImH, int surImW, 
					   pIrisImageInfo pIrisImageL, pIrisImageInfo pIrisImageR, pEyeDetectionInfo detectResults, char *strFunRts);

extern "C" IK_IR_API
	//NOTE: the (0,0) point of pIrisImages is in the down-left corner in a up-right view angle
	int IKIR_JudgeImQuality(HANDLE hIKIRHandle, pIrisImageInfo pIrisImages, int WorkMode, char *strFunRts);

extern "C" IK_IR_API
	//NOTE: the (0,0) point of pIrisImages is in the down-left corner in a up-right view angle
	int IKIR_Loc(HANDLE hIKIRHandle, pIrisImageInfo pIrisInfo, int workMode, char *strFunRts);

extern "C" IK_IR_API
	//NOTE: the (0,0) point of pIrisImages is in the down-left corner in a up-right view angle
	int IKIR_Code(HANDLE hIKIRHandle, pIrisImageInfo pIrisImages, int numIm, int* indexBest, char* strFunRts);

extern "C" IK_IR_API
	int IKIR_Match(HANDLE hIKIRHandle, unsigned char* irisRecTemplate, unsigned char* enrollTemplates, long numEnrollTemplates, MatchInput inputPara, pMatchOutput pMatchOutputPara, char* strFunRts);

extern "C" IK_IR_API
	int IKIR_CheckCompatiblityOfEnrCodes(unsigned char* enrollTemplates,long numEnrollTemplates, long *IncompatiblityIndex,char* strFunRts);

extern "C" IK_IR_API
	int IKIR_GetSuggestTH(long NumOfTemp, float RotateAng, int conservationLevel,  float* pSuggestTh);


extern "C" IK_IR_API
	//NOTE: the (0,0) point of pIrisImage is in the down-left corner in a up-right view angle
	int IKIR_CodeBySmallIm(unsigned char *pIrisImage, int imageH, int imageW, int pupilR, int pupilC, int pupilRd, int irisR, int irisC, int irisRd,unsigned char *enrCodes, int enrCodeLen, unsigned char *recCode, int recCodeLen);
