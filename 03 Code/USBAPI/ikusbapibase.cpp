...
//定义端点地址
enum EndPointTransferType{EndPointIntr =(1 | LIBUSB_ENDPOINT_IN),
                          EndPointData =(2 | LIBUSB_ENDPOINT_IN),
                          CtrlIn =(LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_IN),
                          CtrlOut=(LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT)};
...
int IkUSBDevice::DeviceCount()
{
    ...
    {
        //通过上下文获取设备列表
        error = libusb_get_device_list(_ctx, &devList);
...
int IkUSBDevice::Open()
{
    ...
    _devHandle = libusb_open_device_with_vid_pid(_ctx,VID,PID);
    ...
    if(_devHandle)
    {
        ...
        {
            //创建端点 设置回调
            _bulkInEndPt =  new IkBulkEndPoint(_devHandle);
            _bulkInEndPt->SetCallBack(_ikCallBack,_caller);
            _bulkOutEndPt = new IkBulkEndPoint(_devHandle);
...
*  修改时间：
*********************************************************************************************************************************/
IkUSBEndPoint::IkUSBEndPoint(libusb_device_handle *handle)
{
    _devHandle = handle;
    _timeOut = 5000;
...
*  修改时间：
*********************************************************************************************************************************/
void IkUSBEndPoint::ReleaseTransfer()
{
    if(_devHandle && _transferBuf)
    {
        //释放传输
...
int IkUSBDevice::IkUSBTransferEvent()
{
    if(_ctx)
    {
        return libusb_handle_events(_ctx);
    }
    return E_NO_CONTEXT;
}
/**********************************************************************************************************************************
...
bool IkUSBEndPoint::WaitForTransfer(unsigned long timeOut)
{
    ...
    //    //循环等待 推出条件 超时数据传输完成
    //    while(s_doExit == TransferOnGoing
    //          || ((timeOut==0) || (timeOut < (clock() - startTime))))
    //    {
    //        int error = libusb_handle_events(_ctx);
    //        if(r<0)
    //        {
    //            return false;
    //        }
    ...
    //int actual_length;
    ////同步
    //int error = libusb_bulk_transfer(s_devHandle,0,s_dataBuf,sizeof(s_dataBuf),&actual_length,timeOut);
    //if(r ==0)
    //{
    //	return true;
    //}
...
*  修改时间：
*********************************************************************************************************************************/
IkBulkEndPoint::IkBulkEndPoint(libusb_device_handle *handle):IkUSBEndPoint(handle)
{
