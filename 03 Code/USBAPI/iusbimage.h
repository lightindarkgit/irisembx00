 ...
 *  返 回 值：错误描述, 异常返回  -1超时
 *********************************************************************************************************************************/
    int GetImage(IN IkImageLoadeCallBackFunc callBack,IN const unsigned long outTime);
...
private:
    //USB设备的变量定义
    std::tr1::shared_ptr<IkUSBDevice> _ikDevice;    //设备
    std::tr1::shared_ptr<IkUSBEndPoint> _ikEndPoint; //端点
    IkImageLoadeCallBackFunc _ikCallBack;
    std::tr1::shared_ptr<DealWithUSBTransferBuf> _dealWithUSBTransferBuf;
    //数据buf1
