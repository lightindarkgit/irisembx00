...
int NetClientSelect::Send(char *buf,int len)
{
	...
	{
		errno = 0;
		int cnt = send(this->_socket,buf + ndex,len - ndex,MSG_NOSIGNAL_ERR);
		...
		if (cnt <= 0)
		{
			if(errno == EINTR)
				continue;
			LOG_ERROR("socket send error: %s,  %d",strerror(errno),errno);
			this->_socket = -1;
			return -1;
		}
		else
...
void NetClientSelect::RecvThreadSelect(NetClientSelect *ncs)
{
	...
	while (ncs->_isQuit)
	{
		...
		if (-1 == ready)
		{
			...
			std::cout<<"select operate error,return -1"<<std::endl;
		}

		//超时未发现就绪文件描述符
		if (0 == ready)
		{
			std::cout<<"select operate return 0"<<std::endl;
			continue;
		}

		if (FD_ISSET(ncs->_socket,&socketSet))
		{
			int ret = recv(ncs->_socket, ncs->_bufRecv, g_recvBuf, MSG_NOSIGNAL_ERR);
			...
			else if (0 == ret)
			{
				...
				std::cout<<"select operate scoket closed"<<std::endl;
				break;
			}
			else if (-1 == ret)
			{
				...
				ncs->_socket = -1;
				//ERR_MSG("recv error");
				std::cout<<"select operate recv error"<<std::endl;
				break;
			}
		}
	}
	...
	//退出后关闭网络通信句柄
	std::cout<<"select operate close and exit thread..."<<std::endl;
	close(ncs->_socket);
	ncs->_socket = -1;
