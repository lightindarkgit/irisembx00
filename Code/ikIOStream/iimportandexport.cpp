...
int IImportAndExport::ImportPersonRecLogFromFile(const string fileName)
{
	...
	try
	{
		...
		else
		{
			//防止插重
			int recgIdMax =0;
			dbAccess.Query("select max(reclogid) from personreclog; ",recgIdMax);
...
int IImportAndExport::ImportPersonImageFromFile(const string fileName)
{
	...
	try
	{
		...
		else
		{
			...
			dbAccess.Query("select imageid from personimage; ",imageUUID);
...
int IImportAndExport::ImportPersonFromFile(const string fileName)
{
	...
	try
	{
		...
		else
		{
			...
			dbAccess.Query("select personid from person; ",personUUID);
			dbAccess.Query("select worksn from person; ",vecWorkSN);
