	...
	ui(new Ui::MainWindow)
{
	...
	_timerGoBackIdent = new QTimer;
	connect(_timerGoBackIdent,SIGNAL(timeout()),this,SLOT(GoBackIdent()));

	//Added BY: Wang.L@20151027
	antiScreenSaverTimer_ = new QTimer(this);
	antiScreenSaverTimer_->setSingleShot(false);
	antiScreenSaverTimer_->start(300000);   //Trigger per 5mins
	connect(antiScreenSaverTimer_, SIGNAL(timeout()), this, SLOT(SlotAntiScreenSaver()));
    ...
    LOG_INFO("mainwindow ReLoadIris........................");
    ReLoadIris();
...
void MainWindow::ReLoadIris()
{
	...
	{
        LOG_ERROR("Begin ReLoadIris!")
        _ident->SetReLoadIris(!g_binited);
        g_binited = true;
	}
}
...
void MainWindow::go_identify_action()
{
	...
	//    //李志林修改，如果正在注册则退出注册界面并清理资源，20140613

	//    if(_personManage->enroll->_signaledFlag)
	//    {
	//        _personManage->enroll->ExitEnrollUI();
	//    }
