...
signals:
   ...
   // void sigSavePersonImage(QString str,bool tag);
//    void sigIdentCardID( QString personCardID);
...
private:
    ...
    //图像回调函数
    static int CapIrisImgCallBack(int funResult, unsigned char* pIrisImg, LRIrisClearFlag lrIrisClearFlag, IrisPositionFlag irisPositionFlag, IrisTraceInfo leftTraceInfo, IrisTraceInfo rightTraceInfo);
    ...
    //气息信息回调 add by hatao 2015.4.21
    static void RecvWeatherInfoCallBack(WEATHERINFO &wInfo);
...
private:
    ...
    //算法API
    static CAlgApi * _identApi;


    //灰度图像
    static unsigned char _grayImgBuffer[g_constIrisImgSize + g_constBMPHeaderLength];
...
//    std::thread _identCapImgThread;               //识别模块图像显示线程
    ...
    int   _countCardMode;           //刷卡识别倒计时时长 同时 0： 需要重新刷卡；-1：显示“请刷卡”标签；大于0 ：不需要刷卡
...
private:
    ...
    pthread_t soundThread;    //播放人名线程
     static void *ThreadSound(void *arg);
     static bool _isThreadRun;
