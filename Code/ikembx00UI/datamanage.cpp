...
bool DataManage::initUI()
{
	...
	isPersonInfoChecked =   true;
	isIrisChecked       =   true;
	isRecordChedked     =   true;
	isFacePhotoChedked  =   true;

	/// 设置checkbox初始状态
	on_btnPersonInfo_clicked();
	on_btnIris_clicked();
	on_btnRecord_clicked();
	on_btnFacePho_clicked();
	...
	else
	{
		...
		if(dm->isErroReportChecked)
		{
			if(dm->ExportErroLogs("",""))
			{
				...
				if(strCount <= 1 )
				{
					err = QString::fromUtf8("USB设备已经被拔出");
				}
				else
				{
...
bool DelFile(QString sourceDir)
{
	...
	//        {
	//            QDir *createfile = new QDir;
	//            bool exist = createfile->exists(sourceDir);
	//            if(exist)
	//            {
	//                createfile->remove(sourceDir);
	//            }
...
void DataManage::on_btnPersonInfo_clicked()
{
	Exectime etm(__FUNCTION__);
	QString styleParam = "background-color:transparent;border:none;background-repeat:no-repeat;background-position: center center;";

	if(isPersonInfoChecked)
	{
		isPersonInfoChecked = false;
		ui->btnPersonInfo->setStyleSheet("background-image:url(:/image/personInfo_unchecked.png);"+styleParam);
		if(isIrisChecked)
		{
			on_btnIris_clicked();
		}
	...
	{
		isPersonInfoChecked = true;
		ui->btnPersonInfo->setStyleSheet("background-image:url(:/image/personInfo_checked.png);"+styleParam);
	}

	if(isPersonInfoChecked)
	{
		ui->btnIris->setEnabled(true);

	}
	else
	{
		ui->btnIris->setEnabled(false);
	}
}

void DataManage::on_btnIris_clicked()
{
	Exectime etm(__FUNCTION__);
	QString styleParam = "background-color:transparent;border:none;background-repeat:no-repeat;background-position: center center;";
	if(isIrisChecked)
	{
		isIrisChecked = false;
		ui->btnIris->setStyleSheet("background-image:url(:/image/iris_unchecked.png);"+styleParam);

	}
	else
	{
		/// 只有人员信息被选中时，虹膜勾选框才可被选中
		if(!isPersonInfoChecked)
			return;

		isIrisChecked = true;
		ui->btnIris->setStyleSheet("background-image:url(:/image/iris_checked.png);"+styleParam);
	}
}

void DataManage::on_btnRecord_clicked()
{
	Exectime etm(__FUNCTION__);
	QString styleParam = "background-color:transparent;border:none;background-repeat:no-repeat;background-position: center center;";
	if(isRecordChedked)
	{
		...
		ui->btnRecord->setStyleSheet("background-image:url(:/image/shibiejilu_unchecked.png);"+styleParam);
	}
	else
	{
		isRecordChedked = true;
		ui->btnRecord->setStyleSheet("background-image:url(:/image/shibiejilu_checked.png);"+styleParam);
	}
	slotExportOnlyItemSelectChanged();

}

void DataManage::on_btnFacePho_clicked()
{
	Exectime etm(__FUNCTION__);
	QString styleParam = "background-color:transparent;border:none;background-repeat:no-repeat;background-position: center center;";
	if(isFacePhotoChedked)
	{
		isFacePhotoChedked = false;
		ui->btnFacePho->setStyleSheet("background-image:url(:/image/faceImg-unchecked.png);" +styleParam);
	}
	else
	{
		isFacePhotoChedked = true;
		ui->btnFacePho->setStyleSheet("background-image:url(:/image/faceImg_checked.png);" + styleParam);
	}
