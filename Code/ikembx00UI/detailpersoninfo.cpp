...
void DetailPersonInfo::setData(const PersonInfo& personInfo)
{
	...
	//记录当前人员信息的personID
	perID = personInfo.id;
	QString query("SELECT t1.personid, t1.name, t1.sex, t1.idcard, t1.superid, t1.cardid, t1.worksn, t1.disabletime, t1.departid, t1.departname, t2.hasirisdata, t1.memo "
...
//	ui->btnSave->setEnabled(false);
	...
	//    int size = ui->txtName->text().length();
	//    int size2 = sizeof(person.name);
	//    int size3 = ui->txtName->text().toLocal8Bit().size();
	//    int size4 = ui->txtName->text().toLocal8Bit().length();
	//    qDebug()<<"size"<<size<<"size2"<<size2<<"size3:"<<size3<<"size4"<<size4<<endl;
	...
	if(isUpdateOpt)
	{
		...
		{
			IKMessageBox errorNote(this, msgBoxTitle, QString::fromUtf8("操作失败！"), errorMbx);
...
void DetailPersonInfo::slotCapturePhoto()
{
	...
	//设置人脸图像人员信息
	connect(this, SIGNAL(sigCloseCapturePhtoto()), &_capturePhoto, SLOT(slotBack()));
	_capturePhoto.exec();
}
...
//{
//    QString fileName(getFacePhotoName(perID.c_str()));
//    std::cout << "facephoto path@" << __FUNCTION__ << ":is " << fileName.toStdString() << std::endl;
//    if(QFile::exists(fileName))
//    {
//        QImage localeImage(IMAGE_WIDTH, IMAGE_HEIGHT, QImage::Format_RGB888);
//        localeImage.load(fileName, FACE_IMAGE_FORMAT);
...
bool DetailPersonInfo::addPersonInfo(const Person &pInfo)
{
	...
	{
        IKMessageBox infoNote(this, QString::fromUtf8("保存人员信息"),
                              QString::fromUtf8("工号冲突，请重新分配！"), informationMbx);
	...
	{
        IKMessageBox infoNote(this, QString::fromUtf8("保存人员信息"),
                              QString::fromUtf8("身份证号已占用，请重和对后重试！"), informationMbx);
	...
	{
        IKMessageBox infoNote(this, QString::fromUtf8("保存人员信息"),
                              QString::fromUtf8("IC卡号冲突，请重新读取！"), informationMbx);
...
//    return ui->labFacePhoto->pixmap()->save(fileName, FACE_IMAGE_FORMAT);
//}
...
//*  说    明：将未保存的人脸图像信息存入数据库
//*  参    数：
//*  返 回 值：成功返回数据库操作结果，    dbSuccess     = 0,        // 成功
...
//*  修改时间：
//*****************************************************************************/
//EnumReturnValue DetailPersonInfo::saveFaceImage(DatabaseAccess& da)
//{
//    if(!newImageUnsaved || ui->labFacePhoto->pixmap()->isNull())
//    {
//        return dbSuccess;
...
//        //更新人员信息
//        sql = "UPDATE personimage SET personimage = :personimage, memo = :memo WHERE imageid = :imageid";
//        sqlToTempTable = "INSERT INTO temppersonimage (personid, imageid, operation) VALUES(:personid, :imageid, 2);\n";

//        perImg.imageID = getFacePhotoID(perID);
//        tImg.imageID = perImg.imageID;
//    }
//    else
//    {
...
//    //确认是否已注册人脸图像
//    EnumReturnValue rtnVal = isUpdateFacePhoto ? da.Update(sql.toStdString(), perImg) : da.Insert(sql.toStdString(), perImg);
//    rtnVal = (EnumReturnValue)(rtnVal || da.Insert(sqlToTempTable.toStdString(), tImg));
...
//bool DetailPersonInfo::isPersonInfoValid(bool &valid)
//{
//    DatabaseAccess dbAc;
//    int counter = -1;
//    QStringList conds;

//    //Verify worksn conflict
//    if(!ui->txtWorkSN->text().isEmpty())
//    {
//        //TO DO
//        conds << ui->txtWorkSN->text();
//    }

//    //Verify ID number conflict
//    if(!ui->txtIDNum->text().isEmpty())
//    {
//        //TO DO
//        conds << ui->txtIDNum->text();
//    }

//    //Verify IC card number conflict
//    if(!ui->labICCardNum->text().isEmpty())
//    {
//        //TO DO
//        conds << ui->labICCardNum->text();
//    }

//    if(dbSuccess == dbAc.Query(QString("SELECT count(*) FROM person WHERE %1").arg(conds.join(" OR ")), counter));
//    {
//        if(counter > 0)
//        {
//            valid = false;
...
//    {
//        std::cout << "[" << __FUNCTION__ << "@DetailPersonInfo]"
//                  << "database query person information conflict(pid:" << perID << ") failed!"
//                  << std::endl;
