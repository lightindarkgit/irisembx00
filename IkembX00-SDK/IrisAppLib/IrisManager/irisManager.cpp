...
int	IrisManager::GetPersonFeatureCount(std::string personname,int& featureleftcnt,int& featurerightcnt)
{
    Exectime etm(__FUNCTION__);
    int iRet = -1;
    std::shared_ptr<PeopleNode> p = FindPeopleByName(personname);
    if (NULL == p )
    {
        LOG_INFO("GetPersonFeatureCount: NULL == p,name: [%s], ret: %d",personname.c_str(),iRet);
        return iRet;
    }

	featureleftcnt = p->LeftFeature.size();
	featurerightcnt = p->RightFeature.size();
...
std::shared_ptr<PeopleNode> IrisManager::FindPeopleByName(std::string personname)
{
	Exectime etm(__FUNCTION__);
	PeopleMap::iterator it = _mapPeople.begin();
	while (_mapPeople.end() != it)
	{
		if(personname.compare(it->second->Name) == 0)
		{
			return it->second;
		}
