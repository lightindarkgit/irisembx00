...
void FormHanziSelector::setPinyin(const std::string &pinyin)
{
    ...
    const std::vector<std::string> *v = m_hm.setPinyin(pinyin);
...
void FormHanziSelector::onPageDown()
{
    ...
    {
        std::vector< std::string > *v = m_hm.currentHanziList();
        assert(v != NULL);
...
// 设置_header的汉字
void FormHanziSelector::setBtnHanzi(const std::vector< std::string > &v)
{
    ...
    for( size_t i = 0; i < v.size() && i < pageOfNum; ++i )
    {
        ...
        _header[i]->setText( QString::fromLocal8Bit(v[i].c_str()) );
    }
