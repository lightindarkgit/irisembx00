...
void FormHanziTable::updateUi()
{
    if (_hanzi < _hanziEnd)
    {
        const std::string *pageBegin = _hanzi + _pageNum * _page.size();
        const std::string *pageEnd = pageBegin + _page.size();
        if (pageEnd > _hanziEnd)
            pageEnd = _hanziEnd;

        for (const std::string *pageItem = pageBegin; pageItem < pageEnd; pageItem++)
        {
            QToolButton *btn = _page[pageItem - pageBegin];
            btn->setEnabled(true);
            btn->setText(QString::fromLocal8Bit(pageItem->c_str()));
        }
...
void FormHanziTable::setHanziTable(const std::string *v, const std::string *vEnd)
{
    _hanzi = v;
    _hanziEnd = vEnd;
    _pageNum = 0;
