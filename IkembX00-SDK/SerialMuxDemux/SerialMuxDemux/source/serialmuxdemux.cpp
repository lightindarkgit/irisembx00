...
serialfifo *pmuxfifo; // deanji  20170407
...
//			WriteToBufferForHighCmd(ledoff, 4);   // �˴�����ʱ������⣬��֪��Ϊʲô 20170329  
			WriteToBuffer(ledoff,4);

			std::cout<<"写命令到缓冲区"<<"0x05"<<std::endl;
//			WriteToBufferForHighCmd(cameraoff, 4);
			WriteToBuffer(cameraoff,4);
        		return size;	
		}
...
int SerialMuxDemux::WriteToBuffer(const unsigned char *pBuf, const int dataSize)
{
	...
	for(int i=0;i<dataSize;i++) pmuxfifo->writebytetofifo(pBuf[i]);
    return E_SERIAL_OK;
}
...
int SerialMuxDemux::ReadFromBuffer(unsigned char *data, const int size)
{
	...
	for(i=0;i<size;i++)
	{
		*(ptr+i) = pmuxfifo->readbytefromfifo();
	}
...
int SerialMuxDemux::WriteToBuffer(const unsigned char *pBuf, const int dataSize)
{
    ...
    {
        //TO DO
        _isFull = (_wrPos == _rdPos);    //更新缓冲区满� �志
        _isEmpty = false;
    }
...
int SerialMuxDemux::WriteToBufferForHighCmd(const unsigned char *pBuf, const int dataSize)
{
    ...
    {
        //TO DO
        _isFullHigh = (_wrPosHigh == _rdPosHigh);    //更新缓冲区满� �志
        _isEmptyHigh = false;
    }
    else if(tWrPos < tRdPos)
    {
...
//    unsigned char sendBuf[SEND_ONCE_SIZE];   // deanji   20170407
	unsigned char sendBuf[64];                // deanji   20170407
...
/*    	//liu 优先级高的串口命令
        ...
        if(SEND_ONCE_SIZE == sm->ReadFromBufferHighCmd(sendBuf, SEND_ONCE_SIZE))
        {
        	...
        	//基于串口数据� 输速度限制，在连续发送数据的时候需要短暂休� 防止数据溢出
        	usleep(50000);
			//deanji
