...
//    int rtnVal;
    ...
    while(asyncSerial->IsOpen() && asyncSerial->_isRunning)
    {
        ...
        if(nRead > 0)
        {
//			printf("\n AsyncSerialBase::RecvThread nRead is %d\n",nRead);
...
//			printf("\n asyncSerial->_frameBuf data is %d %d %d %d\n",asyncSerial->_frameBuf[0],asyncSerial->_frameBuf[1],asyncSerial->_frameBuf[2],asyncSerial->_frameBuf[3]);
			if( (asyncSerial->_frameBuf[0] < 5) || (asyncSerial->_frameBuf[0] == 9) || (asyncSerial->_frameBuf[0]>0x0F) )
			{
				asyncSerial->_serialFrame->_rdPos = 0;
				asyncSerial->_serialFrame->_wrPos = 0;
				asyncSerial->_serialFrame->_isEmpty = true;
				asyncSerial->_serialFrame->_isFull = false;
				break;
			}
			
			if(nullptr != asyncSerial->_callback)
            {
                asyncSerial->_callback(asyncSerial->_frameBuf, FRAME_DATA_SIZE + FRAME_CRC_DATA_LEN);    //回调用户的帧数据处理程序
            }
            usleep(1000);
        }
