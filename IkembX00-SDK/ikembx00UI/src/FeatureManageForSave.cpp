...
unsigned char CFeatureManageForDemo::s_enrollCodesL[g_constMaxEnrFeatureNum*IK_ENROLL_FEATURE_SIZE];	//保存左眼注册特征
...
unsigned char CFeatureManageForDemo::s_enrollCodesR[g_constMaxEnrFeatureNum*IK_ENROLL_FEATURE_SIZE];	//保存右眼注册特征
...
void CFeatureManageForDemo::InitParam()
{
    ...
    //应用程序用到的系统空间初始化
    memset(s_enrollCodesL,0,g_constMaxEnrFeatureNum*IK_ENROLL_FEATURE_SIZE);
    memset(s_enrollCodesR,0,g_constMaxEnrFeatureNum*IK_ENROLL_FEATURE_SIZE);
    s_enrNumL = 0;
    s_enrNumR = 0;
...
//    int tempNum;
//    int enrCodeLen = IK_ENROLL_FEATURE_SIZE;
...
void CFeatureManageForDemo::AddTemplate(unsigned char *feature, QString name, FeatureFlag featureFlag)
{
    ...
    {
        //memcpy(s_enrollCodesL+s_enrNumL*IK_ENROLL_FEATURE_SIZE, feature, IK_ENROLL_FEATURE_SIZE);
        memmove(s_enrollCodesL+s_enrNumL*IK_ENROLL_FEATURE_SIZE, feature, IK_ENROLL_FEATURE_SIZE); //dj
        s_strNameLeft[s_enrNumL] = name;
        s_enrNumL++;
    }
    else if(FeatureRight == featureFlag)
    {
        //memcpy(s_enrollCodesR+s_enrNumR*IK_ENROLL_FEATURE_SIZE, feature, IK_ENROLL_FEATURE_SIZE);
        memmove(s_enrollCodesR+s_enrNumR*IK_ENROLL_FEATURE_SIZE, feature, IK_ENROLL_FEATURE_SIZE);//dj
