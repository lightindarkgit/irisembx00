...
extern int G_IsLoadingCodeList;
...
IkSocket::IkSocket()
{
    ...
    _isConnOK= false;
    _isSendingPerson    =   false;
    _isSendingRecLog    =   false;
    _isSendingImageData =   false;
    _isSendingIrisData  =   false;
...
void IkSocket::ReConnect()
{
    ...
    if(g_bsendreconect)
    {
        LOG_WARN("ReConnect request is sended,return");
        return;
    }

    //启动重连
    LOG_INFO("begin _reStartWaiter.signal();");

    _isConnOK = false;
    g_bsendreconect = true;
    _reStartWaiter.signal();
    LOG_INFO("end _reStartWaiter.signal();");
}
/*****************************************************************************
 *                         开始函数
 *  函 数 名：StartScoket
...
bool IkSocket::CreateSocketAPI()
{
    ...
    if(doc.LoadFromFile("ikembx00.xml"))
    {
        ...
        //add dj 20170411
        //         ClientData cd;
        //        const char * value = doc.GetElementValue("/IKEMBX00/configures/server/ip").c_str();
        //        int count = strlen(value)+1;
        //        assert(cd.Addr<value || cd.Addr > (value+count));
        //        strcpy(cd.Addr,value);
        //        cd.Port = atoi(value);
...
void IkSocket::SetConn()
{
    ...
    connect(&_netWork,SIGNAL(sigSendDevInfo( const char*,int)),this,SLOT(SlotSednDevInfo(const char*,int)));
...
int  IkSocket::AddPersonImage(const PersonImage& personImage)
{
    ...
    unsigned char* tmp = new unsigned char[sizeof(PersonPhoto) + personImage.personImage.Size()]; //= (PersonPhoto)personImage;
    PersonPhoto  *netPersonImage = (PersonPhoto*)tmp;
...
int  IkSocket::UpdPersonImage(const PersonImage& personImage)
{
    ...
    _personImageUid.push_back(personImage.imageID.Unparse());
    unsigned char* tmp = new unsigned char[sizeof(PersonPhoto) + personImage.personImage.Size()];
    PersonPhoto  *netPersonImage = (PersonPhoto*)tmp;
    TypeConvert::ConvertPersonImageUp(personImage,netPersonImage);
...
void IkSocket::UploadTemp()
{
    ...
    if(SendImageData()!=0)
    {
        LOG_DEBUG("sending ImageData");
        return;
    }
    //最后发送虹膜数据
    if(SendIrisData()!=0)
    {
        LOG_DEBUG("sending IrisData");
        return;
    }
...
//gdj 20170830
int IkSocket::SendPersonData()
{
    //如果未收到回应则返回 1
    if(_personUid.size()>0)
    {
        LOG_DEBUG("send person not rec 4201");
        return 1;
    }

    LOG_INFO("thread enter 人员更新");
    ////人员更新
    static std::vector<Person> vecAddPerson;
    static std::vector<Person> vecUpdaPerson;
    static std::vector<std::string> vecDelPerson;
    ...
    int result = vecAddPerson.size()+vecUpdaPerson.size()+vecDelPerson.size(); //result ==0 则无person可发。
    vecAddPerson.clear();
    vecUpdaPerson.clear();
    vecDelPerson.clear();
    return result;
...
int IkSocket::SendRecLogData()
{
    //发送后未受到回应则返回 1
    if(_recogId.size()>0)
    {
        LOG_DEBUG("Send RecLog not receive callback");
        return 1;
    }
    ...
    return vecReclog.size();
}

int IkSocket::SendImageData()
{
    if(_personImageUid.size()>0)
    {
        LOG_DEBUG("Send image not receive call back ");
        return 1;
    }
    ///人员图像更新
    ...
    return vecDelPersonImage.size()+vecDelPersonImage.size();
}

int IkSocket::SendIrisData()
{
    if(_irisUid.size()>0)
    {
        LOG_DEBUG("Send iris not receive call back ");
        return 1;
    }
    ///人员虹膜更新
    ...
    return vecAddPersonIrisDataBase.size()+vecDelPersonIrisData.size();
...
int IkSocket::SendData(char *buf,int len)
{
    ...
    unsigned short cmdt = *((short*)(buf + 2));
...
void IkSocket::SlotSednDevInfo( const char *buf, int len)
{

    Exectime etm(__FUNCTION__);
    //@Lock pthread_rwlock_rdlock(&_rwlock);
    if(!_isConnOK)
    {
        LOG_ERROR("_isConnOK 为false");
        //@Lock pthread_rwlock_unlock(&_rwlock);
        return -1;
    }
    ...
    if(cmdt == 0x0301)
    {
        LOG_INFO("call back server request=%d",cmdt);
    }
    ...
    if(nullptr != s_socketAPI)
    {
        ...
        LOG_INFO("SendData开始！cmd:[%s]",tmp);
        std::lock_guard<std::mutex> lock(s_mutex);
        LOG_INFO("SendData开始2！cmd:[%s]",tmp);
        if(s_socketAPI)
        {
            std::cout<<"sendData时间："<<QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss").toStdString()<<std::endl;
...
void * IkSocket::StartSocketThread(void *arg)
{
    ...
    while(true)
    {
        ...
        if(err != 0)
        {
            ...
            LOG_INFO("begin soc->_waiter.signal() wake up update thread;");
            soc->_waiter.signal();  //唤醒数据更新线程

            LOG_INFO("begin soc->_reStartWaiter.wait(); sleep startSocketThread");
            soc->_reStartWaiter.wait();  //睡眠重连线程
            QString reconnectTime = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss");
            LOG_INFO("Start reConnect Time:%s",reconnectTime.toStdString().c_str());
...
void * IkSocket::StartUpdateThread(void *arg)
{
    ...
    while(true)
    {
        ...
        LOG_DEBUG("netSocket:UpdateThread is keep alive");
        if( soc->_isConnOK )
        {
        ...
        {
            LOG_INFO("sleep UpdateThread: soc->_waiter.wait();");
            soc->_waiter.wait();
