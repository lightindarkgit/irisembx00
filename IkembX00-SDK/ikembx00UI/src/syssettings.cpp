...
bool isIPValid(const QString& addr)
{
    ...
    const QRegExp ipPatten("^(((0?0?\\d)|(0?[1-9]\\d)|(1\\d{2})|(2[0-4]\\d)|(25[5]))(\\.((0?0?\\d)|(0?[1-9]\\d)|(1\\d{2})|(2[0-4]\\d)|(25[0-5]))){3})$");
...
// 检查子网掩码是否合法
bool isMaskValid(const QString &mask)
{

    const QRegExp ipPatten("^(((0?0?\\d)|(0?[1-9]\\d)|(1\\d{2})|(2[0-4]\\d)|(25[0-5]))(\\.((0?0?\\d)|(0?[1-9]\\d)|(1\\d{2})|(2[0-4]\\d)|(25[0-5]))){3})$");
    return ipPatten.exactMatch(mask.trimmed());
  ...
  //waitStatus(this, "正在同步系统时间...")
{
    ...
    isWigan26Checked = false;
    isWigan34Checked = false;
...
bool SysSettings::initUI()
{
    ...
    //ui->rbtnCardOrIrisIden->setVisible(false);
...
void SysSettings::slotSaveNetworkSettings()
{
    ...
    {
        if(ui->txtIPAddr->text().isEmpty() ||
                ui->txtMaskCode->text().isEmpty() ||
                ui->txtNetGate->text().isEmpty() ||
                !( isIPValid(ui->txtIPAddr->text()) &&
                   isMaskValid(ui->txtMaskCode->text()) &&
                   isIPValid(ui->txtNetGate->text()) )
                )
        {
...
void SysSettings::slotReboot()
{
    ...
    //TO DO
    //send super cmd
    Interaction::GetInteractionInstance()->SendOffSuperCmd();
...
bool SysSettings::truncateTables()
{
    ...
    QString sqlresetID("SELECT setval('person_id_seq'::regclass, 100)");
    //设置自增ID起始值
    if(dbSuccess != dbAc.Query(sqlresetID.toStdString()))
    {
        LOG_ERROR("卡号自增种子值重设失败");
    ...
    if(tblNamesLst.size())
    {
        ...
        nret = dbAc.Query(sql.toStdString());
        if(nret != dbSuccess)
        {
            LOG_ERROR("Query sql failed. [%s]",sql.toStdString().c_str());
            return false;
    ...
    if(nret != dbSuccess)
    {
        LOG_ERROR("Query sql failed. [select setval('id',100,false)]");
        return false;
    }
...
bool SysSettings::deleteUserLocalFiles()
{
    ...
    //删除备份识别人脸图
    QDir recogFacePhotoPathBK((QCoreApplication::applicationDirPath(),
                               QDir::separator(),
                               "recogFaceBK"));

    //删除识别成功时采集的人脸图像文件
    if(recogFacePhotoPathBK.exists())
    {
        if(rmDir(recogFacePhotoPathBK.absolutePath()))
        {
            LOG_ERROR("faile to delete local face photo captured when identifying!");
            return false;
        }
...
void SysSettings::on_cbxOutToWiegand_clicked(bool checked)
{
    isWigan26Checked = checked;
