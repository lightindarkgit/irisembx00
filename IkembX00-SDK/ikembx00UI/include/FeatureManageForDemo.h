...
public:
    ...
    static unsigned char s_enrollCodesL[g_constMaxEnrFeatureNum*IK_ENROLL_FEATURE_SIZE];	//保存左眼注册特征
    static int s_enrNumL;                                                                 //实际读入系统的左眼注册特征数目
    static QString s_strNameLeft[g_constMaxEnrFeatureNum];                                //用于存储左眼特征名字

    static unsigned char s_enrollCodesR[g_constMaxEnrFeatureNum*IK_ENROLL_FEATURE_SIZE];	//保存右眼注册特征
