...
{
public:
    //Get device's server IP
    static const std::string GetIP();

    //Set device's server IP
    static bool SetIP(const std::string& ip);


    //Get device's server port
    static const std::string GetPort();

    //Set device's server port
    static bool SetPort(const std::string& port);


private:
    static bool saveIP();
    static bool savePort();
    static std::string srvIP;
    static std::string srvPort;
};
