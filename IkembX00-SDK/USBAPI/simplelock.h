...
private:  // emphasize the following members are private
    NoCopy( const NoCopy& ) = delete;//c++11  hulue
...
public:
    //超时等待
   inline bool wait(timespec timeOut)
    {
        _signaled = false;
        pthread_mutex_lock(&_mutex);
        //LOG_INFO("pthread_mutex_lock");
...
//        while (!_signaled)
//        {
            ret = pthread_cond_timedwait(&_cond, &_mutex,&timeOut);
            if(ret == ETIMEDOUT)
            {
//                std::cout << "超时！" << std::endl;
//                std::cout<<"微秒："<<timeOut.tv_nsec<<" 秒："<<timeOut.tv_sec<<std::endl;
                 pthread_mutex_unlock(&_mutex);
                // LOG_INFO("pthread_mutex_unlock");
                return false;
            }
